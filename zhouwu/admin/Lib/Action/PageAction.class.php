...
class PageAction extends BaseAction
{
	public function index()
	{
		...
		$p = new Page($count,20);
		$article_list = $article_mod->where($where)->limit($p->firstRow.','.$p->listRows)->order('add_time DESC,sort ASC')->select();
		...
		$action=u('page/add',array('child'=>$child));
	...
	function edit()
	{
		if(isset($_POST['dosubmit'])){
			...
			if(false !== $result){
				...
				$this->success('修改成功!',U('page/index',array('child'=>$_POST['child'])));
			}else{
				$this->error('操作失败!');
			}
		}else{
	...
	function add()
	{
		if(isset($_POST['dosubmit'])){
			...
			if($result){
				...
				$this->success('添加成功',u('page/index', array('child'=>$_POST['child'])));
			}else{
				$this->error('添加失败',u('page/index', array('child'=>$_POST['child'])));
			}
		}else{
			$child = isset($_GET['child']) && intval($_GET['child']) ? intval($_GET['child']) : $this->error('请选择频道ID');
			//类别开始
	...
	function delete()
    {
		...
		$this->success('删除成功!',u('page/index', array('child'=>$_REQUEST['child'])));
    }
	...
	function status()
	{
		...
		$sql 	= "update ".C('DB_PREFIX')."page set $type=($type+1)%2 where id='$id'";
