...
class UserAction extends EmptyAction {
    ...
    public function vipindex(){
		...
		if($vip==2){
			if(session('vip')<2){
				 $this->error("亲！权限不足，请升级初级VIP权限哦！",u('user/rule'));
			}
			...
			if(session('vip')!=3){
				 $this->error("亲！权限不足，请升级高级VIP权限哦！",u('user/rule'));
			}
	...
	public function apply(){
		...
		if($_POST){
		 ...
		 if($sum>$money){$this->error("提现金额大于余额!",u('user/account'));}	
				...
				if($addtime && ($addtime-time())<86400){
					$this->error("一天只能体现一次!",u('user/account'));
				}
