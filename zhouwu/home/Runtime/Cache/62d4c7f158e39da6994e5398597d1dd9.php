...
<title>修改资料--<?php echo ($set["webname"]); ?></title>
...
function checkmyform(){
    ...
    if(document.myform.email.value.search(/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/) == -1){
        alert('email格式错误！');
        document.myform.email.focus();
        return false;
	}

	if(document.myform.kaihu.value==''){
        alert('请输入开户行');
        document.myform.kaihu.focus();
        return false;
	}
...
<dd><a href="<?php echo u('user/out');?>">退出登录</a></dd>
	...
	<div class="uright fr">
		<div class="userbox">
			<div class="tit">
				<span class="t">修改资料</span>
			</div>
			<div class="con userinfo">
			<form name="myform" action="<?php echo u('user/account');?>" method="post" onSubmit="return checkmyform()" >
			<table border="0" cellpadding="0" cellspacing="0" class="xtable">
				<tr>
					<th width="100">真实姓名：</th>
				  <td><input type="text" name="realname" class="px inp" value="<?php echo ($user_info["realname"]); ?>" style="width:200px;" <?php if($user_info['realname'] != ''):?>readonly<?php endif;?> /> 
			      <span class="STYLE1">*</span><font color="red">注意：绑定后无法修改</font></td>
				</tr>
				<tr>
					<th>Email：</th>
				  <td><input size="60" type="text" name="email" class="px inp" value="<?php echo ($user_info["email"]); ?>" style="width:200px;" />
				    <span class="STYLE1">*</span></td>
				</tr>
				<tr>
					<th>QQ：</th>
					<td><input size="60" type="text" name="qq" class="px inp" value="<?php echo ($user_info["qq"]); ?>" style="width:200px;" /></td>
				</tr>
				<tr>
					<th>电话：</th>
					<td><input size="60" type="text" name="tel" class="px inp" value="<?php echo ($user_info["tel"]); ?>" style="width:200px;" /></td>
				</tr>
				<tr>
					<th>地址：</th>
					<td><input size="60" type="text" name="address" class="px inp" value="<?php echo ($user_info["address"]); ?>" style="width:200px;" /></td>
				</tr>
				<tr>
					<th>开户行：</th>
				  <td><input size="60" type="text" name="kaihu" class="px inp" value="<?php echo ($user_info["kaihu"]); ?>" style="width:200px;" <?php if($user_info['kaihu'] != ''):?>readonly<?php endif;?>/>
				    <span class="STYLE1">*</span><font color="red">【支持所有银行和第三方支付】注意：绑定后无法修改</font></td>
				</tr>
				<tr>
					<th>卡号：</th>
				  <td><input size="60" type="text" name="kahao" class="px inp" value="<?php echo ($user_info["kahao"]); ?>" style="width:200px;" <?php if($user_info['kahao'] != ''):?>readonly<?php endif;?>/>
				    <span class="STYLE1">*</span><font color="red">【请填写银行卡号或支付宝帐号】注意：绑定后无法修改</font></td>
				</tr>
				<tr>
					<th></th>
